...
public interface CourseRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
	...
	@Query(value = "select case when count(tcu) > 0 THEN true else false END from tb_courses_users tcu where tcu.course_id= :courseId and user_id= :userId", nativeQuery = true)
